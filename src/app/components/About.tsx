...
export const About = () => {
  return (
    ...
    >
      ...
      {/** Card and skills**/}
      <div className="flex flex-col sm:flex-row items-center justify-center gap-15 mt-20 mx-auto max-w-6xl mb-8 px-4 ">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.75, delay: 0.2, ease: "circInOut" }}
          viewport={{ once: true }}
          className="w-full sm:w-85 p-4 border border-gray-400 rounded-xl shadow-md space-y-4 bg-white/20 backdrop-blur-md"
        >
        ...
        <motion.div
          ...
          transition={{ duration: 0.75, delay: 0.2, ease: "circInOut" }}
          viewport={{ once: true }}
          className="w-full bg-white/20 backdrop-blur-md sm:w-80 lg:w-96 xl:w-[700px] flex flex-col p-5 border border-gray-400 rounded-xl shadow-md space-y-4"
        >
          <h3
      ...
      <motion.div initial={{opacity: 0 }} whileInView={{opacity:1}} viewport={{once:true}}
                  transition={{duration:0.5,delay:0.2}} className=" max-w-5xl text-center px-8 text-wrap mb-8">
        <TypewriterText text="This is my favorite idol, a singer whose music I absolutely love. I’ve really immersed myself in her songs, and every time I listen, I feel inspired and connected. Her voice and passion for music make me admire her even more."
               speed={20}         className="font-semibold  sm:text-xl mb-3 text-gray-700"/>
      </motion.div>
    </section>
  );
};
